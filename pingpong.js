...
const scoreTable = document.querySelector("#scoreList")
const thSelectAll = document.getElementsByTagName("th")
function createTag(p) {
const newScore = document.createElement("th")
newScore.innerText = `+1 ${p.input.value}`
newScore.setAttribute('class', `${p.btnTag}`)
newScore.classList.add("ml-1")
newScore.classList.add("mt-1")
scoreTable.appendChild(newScore)
    if (p.input.value === "") {
        newScore.innerText = `+ 1 Player ${p.num[1]}`
    }
}
